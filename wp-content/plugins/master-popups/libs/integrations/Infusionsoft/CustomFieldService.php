<?php
class Mpp_Infusionsoft_CustomFieldService extends Mpp_Infusionsoft_DataService{
    ...
    public static function getCachedCustomFields(Mpp_Infusionsoft_Generated_Base $object, $dataType = null, $ttl = 43200 /*12 Hours*/, Mpp_Infusionsoft_App $app = null){
        if(!property_exists($object, 'customFieldFormId')){
            throw new Mpp_Infusionsoft_Exception(get_class($object) . ' does not have Custom Fields.');
        }

        $dataFormField = new Mpp_Infusionsoft_DataFormField();
        ...
        $cache = new Mpp_Infusionsoft_ObjectCache($dataFormField, $conditions, $ttl);
        $out = $cache->getData();

        return $out;
    }

	public static function getCustomFields(Mpp_Infusionsoft_Generated_Base $object, $dataType = null, Mpp_Infusionsoft_App $app = null){
        if(!property_exists($object, 'customFieldFormId')){
            throw new Mpp_Infusionsoft_Exception(get_class($object) . ' does not have Custom Fields.');
        }
		...
		$out = parent::query(new Mpp_Infusionsoft_DataFormField(), $conditions, 1000, 0, false, $app);

		return $out;
	}

    public static function getCustomField(Mpp_Infusionsoft_Generated_Base $object, $name, Mpp_Infusionsoft_App $app = null){
        if(strpos($name, '_') === 0){
    ...
    public static function getAllValuesFor($field_name){
        ...
        $fields = Mpp_Infusionsoft_ObjectTools::findObjectsInList(Mpp_Infusionsoft_CustomFieldService::getCachedCustomFields(new Mpp_Infusionsoft_Contact()), array('Name' => $field_name));
