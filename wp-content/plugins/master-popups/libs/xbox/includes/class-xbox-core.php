...
class XboxCore {
    ...
    */
    public function set_args( $args = array() ){
        $default_args = array(
            ...
            'saved_message' => esc_html__( 'Settings updated', 'xbox' ),
            'reset_message' => esc_html__( 'Settings reset', 'xbox' ),
            'import_message' => esc_html__( 'Settings imported', 'xbox' ),
            'import_message_error' => esc_html__( 'There were problems importing the data. Please try again.', 'xbox' ),
            'insert_before' => '',
            'insert_after' => '',
            'css_options' => array(),
        );
        ...
        $this->args['form_options'] = wp_parse_args( $this->args['form_options'], array(
            ...
            'save_button_text' => esc_html__( 'Save Changes', 'xbox' ),
            'save_button_class' => '',
            'reset_button_text' => esc_html__( 'Reset to Defaults', 'xbox' ),
            'reset_button_class' => '',
    ...
    */
    public function get_form_buttons( $form_options = array() ){
        ...
        if( $args['show_save_button'] ){
            ...
            $save_btn = "<button type='submit' name='{$args['save_button_name']}' id='{$args['save_button_id']}' class='xbox-save-btn xbox-form-btn xbox-btn xbox-btn-{$this->arg( 'skin' )} {$args['save_button_class']}'>{$args['save_button_text']}</button>";
        }
    ...
    */
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public function show_message_field( $args = array(), &$parent_object = null ){
        $field_args = array();
        $field_args['type'] = 'html_content';
        $field_args['content'] = xbox_show_alert_message( $args );
        return $this->add_field( $field_args, $parent_object );
    }
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public function add_select_sortable( $field_args = array(), &$parent_object = null ){
        $field_args['type'] = 'select';
        $field_args['multiple'] = true;
        $field_args['sort_by_values'] = false;

        if( ! isset( $field_args['id'] ) ){
            return $this->show_message_field( array(
                'type' => 'error',
                'close' => true,
        ...
        if( empty( $field_args['extra_fields'] ) ){
            $field_id = $field_args['id'];
            return $this->show_message_field( array(
                'type' => 'error',
                'close' => true,
                'message' => "The field id '$field_id' requires the parameters 'extra_fields'"
            ), $parent_object );
        }

        $items_arg = $field_args['items'];
        $items = array();
        $items_data = array();
        $sort_by_date = false;
        foreach( $items_arg as $key => $data ){
            if( is_array( $data ) ){
                $items_data[$key] = $data;
                if( isset( $data['image_url'] ) ){
                    $items[$key] = "<img class='ui mini avatar image' src='{$data['image_url']}'> {$data['name']}";
                } else {
                    $items[$key] = $data['name'];
                }
                $sort_by_date = isset( $data['date'] );
            } else {
                $items[$key] = $data;
        ...
        $field_args['items'] = $items;
        $field_args['items_data'] = $items_data;

        $extra_fields = $field_args['extra_fields'];
        $sort_by_field = $extra_fields['sort_by'];
        $sortable_field = $extra_fields['sortable'];

        $field_args['attributes'] = isset( $field_args['attributes'] ) ? $field_args['attributes'] : array();
        $field_args['attributes']['data-field-selectable'] = $this->fields_prefix.$field_args['id'];
        $field_args['attributes']['data-field-sort_by'] = $this->fields_prefix.$sort_by_field['id'];
        $field_args['attributes']['data-field-sortable'] = $this->fields_prefix.$sortable_field['id'];
        $field_args['attributes']['class'] = isset( $field_args['attributes']['class'] ) ? $field_args['attributes']['class'] : '';
        $field_args['attributes']['class'] = $field_args['attributes']['class'].' xbox-select-sortable';
        $select_items = $this->add_field( $field_args, $parent_object );

        $sort_by_field['type'] = 'select';
        $sort_by_field['default'] = ! empty( $sort_by_field['default'] ) ? $sort_by_field['default'] : 'drag_drop';
        $sort_by_field['attributes'] = isset( $sort_by_field['attributes'] ) ? $sort_by_field['attributes'] : array();
        $sort_by_field['attributes']['data-field-selectable'] = $this->fields_prefix.$field_args['id'];
        $sort_by_field['attributes']['data-field-sort_by'] = $this->fields_prefix.$sort_by_field['id'];
        $sort_by_field['attributes']['data-field-sortable'] = $this->fields_prefix.$sortable_field['id'];
        $sort_by_field['options'] = isset( $sort_by_field['options'] ) ? $sort_by_field['options'] : array();
        $sort_by_random = ! empty( $sort_by_field['options']['sort_by_random'] );
        if( empty( $sort_by_field['items'] ) ){
            $sort_items = array(
                'drag_drop' => esc_html__( 'Drag and Drop', 'xbox' ),
                'name_asc' => esc_html__( 'Sort by name (ASC)', 'xbox' ),
                'name_desc' => esc_html__( 'Sort by name (DESC)', 'xbox' ),
            );
            if( $sort_by_date ){
                $sort_items['date_asc'] = esc_html__( 'Sort by date (ASC)', 'xbox' );
                $sort_items['date_desc'] = esc_html__( 'Sort by date (DESC)', 'xbox' );
            }
            if( $sort_by_random ){
                $sort_items['random'] = esc_html__( 'Random', 'masterteam' );
            }
            $sort_by_field['items'] = $sort_items;
        }

        $select_sort = $this->add_field( $sort_by_field, $parent_object );


        //De tipo texto para que se muestre el div con la lista de items a ordenar aunque al renderizar el input será hidden
        //Se renderiza en class-field-types.php método text()
        $sortable_field['type'] = 'text';
        $sortable_field['attributes'] = isset( $sortable_field['attributes'] ) ? $sortable_field['attributes'] : array();
        $sortable_field['attributes']['data-field-selectable'] = $this->fields_prefix.$field_args['id'];
        $sortable_field['attributes']['data-field-sort_by'] = $this->fields_prefix.$sort_by_field['id'];
        $sortable_field['attributes']['data-field-sortable'] = $this->fields_prefix.$sortable_field['id'];
        $sortable_field['options'] = isset( $sortable_field['options'] ) ? $sortable_field['options'] : array();
        $sortable_field['options']['show_edit_link'] = isset( $sortable_field['options']['show_edit_link'] ) ? $sortable_field['options']['show_edit_link'] : false;
        $sortable_field['options']['show_sortable_list'] = true;
        $sortable_field['items_data'] = $items_data;
        $input_hidden_field = $this->add_field( $sortable_field, $parent_object );
        return $select_items;
    }
    ...
    */
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public function get_field( $field_id = '', $parent_object = null, $nested_search = false ){
        ...
        if( $nested_search ){
             foreach( $object->fields_objects as $key => $field ){
                 if( isset( $field->fields_objects[$field_id] ) ){
                     return $field->fields_objects[$field_id];
                 }
             }
        }
        return null;
    ...
    */
    public function is_valid_field( $field_args, $fields = array() ){
        if( isset( $field_args['type'] ) && empty( $field_args['id'] ) ){
            if( in_array( $field_args['type'], array( 'title', 'button', 'html', 'html_content', 'section' ) ) ){
                $field_id = Functions::random_string( 15 );
                $field_args['id'] = $field_id;
            }
        }
    ...
    */
    public function build_header(){
        ...
        $style = "<style>";
        $style .= "
			.xbox-postbox#{$this->id} > .postbox-header,
