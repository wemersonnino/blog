    ...
    */
    public function colorpicker( $type = '' ){
        ...
        if( $this->field->arg( 'options', 'show_default_button' ) ){
            $return .= "<div class='xbox-colorpicker-default-btn' title='" . esc_html__( 'Set default color', 'xbox' ) . "'>";
    ...
    */
    public function import( $type = '' ){
        ...
        if( $options['import_from_url'] ){
            ...
            $placeholder = esc_html__( 'Enter a valid json url', 'xbox' );
            $return .= "<input type='text' name='xbox-import-url' placeholder='$placeholder'>";
            $return .= "</div>";
        }
        if( $options['import_from_json'] ){
            $return .= "<div class='xbox-wrap-textarea-json'>";
            $placeholder = esc_html__( 'Enter a valid json text', 'xbox' );
            $return .= "<textarea name='xbox-import-json' placeholder='$placeholder' rows='5'></textarea>";
            $return .= "</div>";
        }
...
<div class="xbox-row xbox-clearfix xbox-type-mixed xbox-show" style="margin-left: -25px; margin-bottom: 15px;">
    ...
    */
    public function image_selector( $type = '' ){
        ...
        foreach( $items as $key => $image ){
            ...
            if( ( $key == 'from_file' || $key == 'from_url' || $key == 'from_json' ) && ( $options['import_from_file'] || $options['import_from_url'] || $options['import_from_json'] ) ){
        ...
        if( $type == 'checkbox' ){
            ...
            if( ! Functions::is_empty( $value ) ){
                foreach( $value as $key ){
                    $return .= isset( $temp[$key] ) ? $temp[$key] : "";
                    unset( $temp[$key] );
                }
            }
        }
    ...
    */
    public function text( $type = '' ){
        $return = '';
        $options = $this->field->arg( 'options' );
        if( isset( $options['show_sortable_list'] ) ){
            $return .= $this->build_input( 'hidden' );
        } else {
            $return .= $this->build_input( 'text' );
        }
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public function sortable_list( $type = '' ){
        $return = "<div class='xbox-sortable-items' data-show-edit-link='{$this->field->arg( 'options', 'show_edit_link' )}'>";
        $number = 0;
        $selectable_field_id = $this->field->arg( 'attributes', 'data-field-selectable' );
        $parent = $this->field->get_parent();
        $selectable_field = $parent->get_field( $selectable_field_id );
        $selected_keys = (array) $selectable_field->get_value(true);
        $sortable_keys = $this->field->get_value( true );
        $sortable_keys = array_filter( explode(',', $sortable_keys) );
        $items_data = $this->field->arg( 'items_data' );
//        d($sortable_keys);
//        d($items_data);
//        d($selected_keys);

        foreach( $sortable_keys as $key ){
            $item = isset( $items_data[$key] ) ? $items_data[$key] : array();
            if( empty( $item ) || ! in_array( $item['key'], $selected_keys) ) continue;
            $number++;
            $index = $number - 1;
            $return .= "<div class='xbox-item-sort' data-key='{$item['key']}' data-index='$index'>";
            if( isset( $item['image_url'] ) ){
                $return .= "<img class='xbox-item-sort-img' src='{$item['image_url']}'>";
            }
            $return .= "<span class='xbox-item-sort-name'>{$item['name']}</span>";
            $return .= "<span class='xbox-item-sort-number'>$number</span>";
            $return .= "<i class='xbox-icon xbox-icon-close xbox-item-sort-remove' title='Remove'></i>";
            if( $this->field->arg( 'options', 'show_edit_link' ) ){
                $link = get_edit_post_link($item['key']);
                $return .= "<a href='{$link}' class='xbox-item-sort-edit' target='_blank' title='Edit'><i class='xbox-icon xbox-icon-pencil'></i></a>";
            }
            $return .= "</div>";
        }
        $return .= "</div>";
    ...
    */
    public function build_input( $type = 'text', $value = '', $attributes = array(), $escaping_function = 'esc_attr', $exclude_attributes = array() ){
        ...
        $element_attributes = array(
            ...
            'id' => Functions::get_id_attribute_by_name( $this->field->get_name() ),
            'value' => $value,
            'data-initial-value' => $value,//Valor inicial al cargar la página. Solo se usa este por el momento
    ...
    */
    public function build_select( $type = 'select', $value = '', $attributes = array(), $escaping_function = 'esc_attr' ){
        ...
        if( $options['sort'] ){
            ...
            //Para mostrar los items en el orden en que se seleccionaron
            if( ! empty( $value ) && $options['multiple'] === true ){
                $new_items = array();
                foreach($value as $val){
                    if( isset( $items[$val] ) ){
                        $new_items[$val] = $items[$val];
                        unset($items[$val] );
                    }
                }
        ...
        if( $options['multiple'] === true ){
            ...
            if( $options['sortable'] === true ){
                $dropdown_class .= " xbox-sortable-values";
            }
        }
        ...
        $attributes['data-items_data'] = $this->field->arg( 'items_data' );
        ...
        $return .= "<input type='hidden' name='{$name}' value='$value' data-initial-value='$value'>";
