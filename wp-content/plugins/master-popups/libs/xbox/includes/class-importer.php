...
class Importer {
    ...
    */
    public function get_import_xbox_data(){
        ...
        switch( $import_from ){
            case 'from_file':
                ...
                //Si falla la importanción, intentar tomar el contenido desde el textarea
                if( ! $json_xbox_data && isset( $data['xbox-import-json'] ) ){
                    $json_xbox_data = $this->get_json_from_textarea( $data );
                }
                break;
    ...
    /*
    |---------------------------------------------------------------------------------------------------
    | Importa contenido desde el textarea. Cuando se activa el input radio "From json text"
    |---------------------------------------------------------------------------------------------------
    */
    public function get_json_from_textarea( $data = array() ){
        $json_xbox_data = htmlspecialchars( $data['xbox-import-json'], ENT_QUOTES, 'UTF-8' );
        $json_xbox_data = stripslashes( $json_xbox_data );
        $json_xbox_data = htmlspecialchars_decode( $json_xbox_data, ENT_QUOTES );
        return $json_xbox_data;
    }
    ...
    private function get_json_from_url( $url ){
        ...
        if( $json_decode === null ){
            ...
            if( is_wp_error( $response ) ){
                //Evita error 404. Si agrego ssl false directamente a sitios seguros con puerto 443 da error
                if( Functions::get_protocol() != 'https' ){
