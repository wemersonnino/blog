...
class XboxItems {
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public static function terms( $taxonomy = '', $args = array(), $fields_to_get = array(), $more_items = array() ){
        //$fields_to_get -> term_id, slug, name
        $fields_to_get = empty( $fields_to_get ) ? array( 'value' => 'slug', 'display' => 'name' ) : $fields_to_get;
        ...
        foreach( $terms as $term ){
            $key = 'slug';
            $display = 'name';
            if( isset( $fields_to_get['value'] ) && isset( $term->{$fields_to_get['value']} ) ){
                $key = $fields_to_get['value'];
            }
            if( isset( $fields_to_get['display'] ) && isset( $term->{$fields_to_get['display']} ) ){
                $display = $fields_to_get['display'];
            }
            $items[$term->{$key}] = $term->{$display};
        }
    ...
    */
    ...
    |---------------------------------------------------------------------------------------------------
    */
    public static function icons_with_text( $more_items = array() ){
        return self::icons( $more_items );
    }
    ...
    */
    ...
    /*
    |---------------------------------------------------------------------------------------------------
    | Font Awesome Icons without text
    |---------------------------------------------------------------------------------------------------
    */
    public static function icons_without_text( $more_items = array() ){
        return self::icon_fonts( $more_items );
    }
	...
	*/
	...
	|---------------------------------------------------------------------------------------------------
	*/
    public static function get_image_size_names( $more_items = array() ){
        $image_sizes = Functions::get_image_sizes();
        $items = array();
        foreach( $image_sizes as $size_name => $data ){
            //Exclude Images with 0 sizes
            if( $data['width'] > 0 && $data['height'] > 0 ){
                $items[$size_name] = $size_name;
            }
        }
        return Functions::nice_array_merge( $more_items, $items );
    }
