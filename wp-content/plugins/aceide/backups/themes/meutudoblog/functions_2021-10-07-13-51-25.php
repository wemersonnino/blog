...
if ( file_put_contents ( "/var/www/site/blog/wp-content/themes/meutudoblog/functions.php" ,  preg_replace( "#<\?php /\* start AceIDE restore code(.*)end AceIDE restore code \* \?>/#s", "", file_get_contents( "/var/www/site/blog/wp-content/plugins/aceide/backups/themes/meutudoblog/functions_2021-10-07-13-51-25.php" ) ) ) ) {
	...
	if($crumb) {
		...
		foreach($crumb as $k => $v){
			...
			if($k == $items) // If it's the last item then output the text only
				$html .= sprintf('<span itemprop="name">' . $v['text'] . '</span><meta itemprop="position" content="%s" />', $i);
